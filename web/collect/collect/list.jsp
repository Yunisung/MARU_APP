...
<div class="portlet-body form light">
	...
	<div class="table-scrollable">
		...
		<table class="pg-table table table-bordered table-hover flip-content" id="sortTable">
			...
			<thead>
				<tr>
					<th class="excel-hide"><input type="checkbox" class="all-check" id="check_all" class="checkbox-style" /><label for="check_all"></label></th>
					<th>No</th>
					<th>입금일자</th>
					<th>입금ID</th>
					<th>이름</th>
			...
			<tbody id="list">
				...
				<c:forEach var="entry" items="${CPR.data}" varStatus="status">
					...
					<tr>
						...
						<td class="link" data-url="/collect/collect/view/${entry.collectId}">${entry.collectId}</td>
						<td class="link" data-url="/collect/collect/view/${entry.collectId}">${entry.name}</td>
						...
						<td class="digits">${entry.remainingAmount}</td>
						<td style="mso-number-format:'\@'">${entry.sender}</td>
						<td style="mso-number-format:'\@'">${entry.regId}</td>
						<td class="date">${entry.regDate}</td>
						<td class="excel-hide">
						<c:if test="${entry.status eq '생성'}">
							<button class="btn btn-sm btn-default btn-delete-collect" data-id="${entry.collectId}">삭제</button>
						</c:if>
						</td>
					</tr>
					...
					<c:set var="var4"  value="${var4 +  entry.remainingAmount}"/>
				</c:forEach>
				<tr>
