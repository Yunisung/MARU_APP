...
<body class="page-header-fixed page-sidebar-closed-hide-logo page-content-white">
								...
								<div class="page-content-inner" id="search-container">
									...
									<div class="portlet light portlet-form">
										<div class="portlet-body form light">
											<form class="form-horizontal" role="form" data-form="true" id="searchForm" name="searchForm" action="/diff/make/list" method="post">
												<div class="form-body">
													<div class="row">
	...
	<script type="text/javascript">
		...
		function calcAmt(){
			...
			$('.diffrenceAmt').each(function() {
				var amt1 = $(this).parent().find('.out-col-stlDiffVanAmt').attr('data-amt');
				var amt2 = String($(this).parent().find('.collect-input').val()).replace(/,/g, '');
			...
			$('#collect-sum').text('0');
			$('.out-col-stlDiffVanAmt').each(function() {
			...
			$('.collect-input').each(function() {
				$('#real-sum').text(Number($('#real-sum').text()) + Number(String($(this).val()).replace(/,/g, '')));
			});
		...
		function collectDtlMake(stlVanDay, mchtId, vanId) {
			...
			$('#searchResult tr').each(function() {
				if($(this).find('.out-col-mchtId').text() == '') {
					...
					outColObj.stlDiffAmt = $(this).find('.out-col-stlDiffAmt').text().replace(/,/gi, "");
					outColObj.stlDiffVanAmt = $(this).find('.out-col-stlDiffVanAmt').text().replace(/,/gi, "");
					outColObj.diffSettleAmt = $(this).find('.out-col-diffSettleAmt').text().replace(/,/gi, "");
					outColObj.collectAmount = String($(this).find('.collect-input').val()).replace(/,/gi, "");
					outColObj.deductAmount = $(this).find('.diffrenceAmt').text().replace(/,/gi, "");
      ...
      if (result) {
        $.ajax({
          url: '/diff/decide/${COLLECT_MAP.collectId }',
          dataType : "text",
          ...
          success: function(res, stat) {
            res = JSON.parse(res);
            if(res.result == 'OK') {
							bootbox.alert("차액입금정산 상세 내역 반영에 성공하였습니다.");
							location.reload();
            } else {
              bootbox.alert("차액입금정산 상세 내역 반영에 실패하였습니다.\n" + res.msg);
            }
          },
		...
		function collectDtlFinish() {
			bootbox.confirm('입금정산을 마감하시겠습니까?', function(result) {
				if (result) {
			    $.ajax({
			      url: '/diff/finish/${COLLECT_MAP.collectId }',
			      dataType : "text",
			      method:'get',
			      success: function(res, stat) {
			        res = JSON.parse(res);
			        if(res.result == 'OK') {
			          bootbox.alert("차액입금정산을 마감 하였습니다.", function () {
									document.location.href = '/diff/collect/form';
								});
								
			        } else {
			          bootbox.alert("차액입금정산 마감에 실패하였습니다.\n" + res.msg);
			        }
			      },
			      error : function(xhr, status, error) {
			        bootbox.alert("차액입금정산 마감에 실패하였습니다. (시스템 에러)");
			      }
			    });
				}
			});
