		...
		<table class="pg-table table table-bordered table-hover flip-content" id="sortTable">
			...
			<thead>
				<tr>						
					...
					<th>매입금액</th>
					<th>본사차액정산액</th>
					<th>카드사입금예정액<br><span class="digits" id="collect-sum"></span></th>
					<th>정산차액</th>
			...
			<tbody id="list">
				...
				<c:forEach var="entry" items="${CPR.data}" varStatus="status">
					<tr>
						...
						<td class="out-col-stlDiffAmt"><fmt:formatNumber type="number" value="${entry.stlDiffAmt}" pattern="#,##0" /></td>
						<td class="out-col-stlDiffVanAmt" data-amt="${entry.stlDiffVanAmt}"><fmt:formatNumber type="number" value="${entry.stlDiffVanAmt}" pattern="#,##0" /></td>
						<td class="out-col-diffSettleAmt"><fmt:formatNumber type="number" value="${entry.MARUAmt}" pattern="#,##0" /></td>
						<td class="out-col-collectAmount"><input class="collect-input collectAmount${entry.mchtId}" name="collect-${entry.stlVanDay}-${entry.vanId}" placeholder="실입금액" value="<c:if test="${entry.isSaved != 'TRUE'}">${entry.stlDiffVanAmt}</c:if><c:if test="${entry.isSaved == 'TRUE'}">${entry.savedCollectAmount}</c:if>"></td>
						<td class="diffrenceAmt" data-amt=""></td>
						...
						<td>
							<c:if test="${entry.isSaved != 'TRUE'}">
								<a class="btn btn-sm purple-plum" href="javascript:collectDtlMake('${entry.stlDiffVanDay}','${entry.mchtId}','${entry.vanId}')">저장</a>
							</c:if>
							<c:if test="${entry.isSaved == 'TRUE'}">
								<a class="btn btn-sm green-sharp" href="javascript:collectDtlMake('${entry.stlDiffVanDay}','${entry.mchtId}','${entry.vanId}')">수정</a>
							</c:if>	
						</td>
					</tr>
				</c:forEach>
...
<script type="text/javascript">
	<c:forEach var="entry" items="${CPR.data}" varStatus="status">
		$('.collectAmount${entry.mchtId}').val(addComma(String($('.collectAmount${entry.mchtId}').val())));
		
		$('.collectAmount${entry.mchtId}').keyup(function(){
			$('.collectAmount${entry.mchtId}').val(addComma(String($('.collectAmount${entry.mchtId}').val()).replace(/,/g, '').replace(/[^(-?)0-9]/g,"")));
		});
