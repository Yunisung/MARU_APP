			...
			<tbody id="list">
				...
				<c:forEach var="entry" items="${CPR.data}" varStatus="status">
					<tr>
						...
						<td>${entry.seqNo}</td>
						<td>${entry.sendAccount}</td>
						<td>${entry.recvBankName}</td>
						<td>${entry.recvAccount}</td>
						<td class="right"><fmt:formatNumber type="number" value="${entry.amount}" pattern="#,##0" /></td>
						<td class="right"><fmt:formatNumber type="number" value="${entry.failAmount}" pattern="#,##0" /></td>
						...
						<td>
							<c:choose>
								...
								<c:when test="${entry.procGb == 'X' }">
									<font color="red">장애</font>
								</c:when>
								<c:when test="${entry.procGb == 'N'}">
									<font color="green">실패</font>
								</c:when>
								<c:otherwise>
									<font color="green">${entry.procGb }</font>
								</c:otherwise>
							</c:choose>
						</td>
