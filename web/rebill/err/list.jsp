        ...
        <table class="pg-table table table-striped table-hover flip-content" id="sortTable">
            ...
            <c:forEach var="entry" items="${CPR.data}" varStatus="status">
                <tr>
                    ...
                    <td class="link_modal" data-url="/trx/err/view/${entry.trxId}">${entry.trxId}</td>
                    ...
                    <td><fmt:formatDate type="both" value="${entry.reqDate}" pattern="yy/MM/dd HH:mm:ss" /></td>
                    <td>${entry.issuer}</td>
                    <td>
                        <c:choose>
                            <c:when test="${CP_SESSION.grade == '본사'}">
                                ${entry.distName}  &lt;${entry.agencyName}
                            </c:when>
                            <c:when test="${CP_SESSION.grade == '대행사'}">
                                ${entry.agencyName}
                            </c:when>
                            <c:when test="${CP_SESSION.grade == '에이전시'}">
                                ${entry.salesName}
                            </c:when>
                            <c:otherwise>
                                ${entry.agencyName}
