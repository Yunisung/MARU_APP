			...
			<tbody id="list">
				...
				<c:forEach var="entry" items="${CPR.data}" varStatus="status">
					<tr>
						...
						<td class="link" data-url="/tms/app/modify/${entry.version}">${entry.version}</td>
						<td>${entry.status}</td>
						<td>${entry.description}</td>
						<td class="date">${entry.upDate}</td>
						<td>${entry.regId}</td>
						<td class="date">${entry.regDate}</td>
						<td class="link" data-url="/tms/app/upload/${entry.version}">APK 업로드</td>
						<c:if test="${entry.apk ne ''}">
							<td class="link download-link" data-url="/uploadFile/apk/get/${entry.version}">다운로드</td>
						</c:if>
						<c:if test="${entry.apk eq ''}">
							<td></td>
						</c:if>
					</tr>
				</c:forEach>
