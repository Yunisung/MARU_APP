	...
	<div class="actions">
		<div class="btn-group">
			...
			<ul class="dropdown-menu pull-right">
				...
				<li><a href="javascript:;" class="vactSettle-pay-out-check">
						<i class="fa fa-check-square-o"></i> 선택항목 확정 처리
					</a>
				</li>
				</c:if>
				<li><a href="javascript:;" class="vactSettle-pay-complete">
		...
		<div class="btn-group">
			...
			<ul class="dropdown-menu pull-right">
				<li><a href="javascript:;" class="vactSettle-pay-out-make" data-bank="020">
						<i class="fa fa-check-square-o"></i> 우리은행
					</a>
				</li>
				<li><a href="javascript:;" class="vactSettle-pay-out-make" data-bank="004">
						<i class="fa fa-check-square-o"></i> 국민은행
					</a>
				</li>
				<li><a href="javascript:;" class="vactSettle-pay-out-make" data-bank="081">
		...
		<table class="pg-table table table-bordered table-hover flip-content" id="sortTable">
			...
			<thead>
				<tr>
					...
					<th>정산번호</th>
					<th>정산일</th>
					<th style="min-width:150px;">에이전시명</th>
					<th>확정 상태</th>
					<th>지불 상태</th>
					<th>대상 거래 기간</th>
					<th>입금건수</th>
			...
			<tbody id="list">
				...
				<c:forEach var="entry" items="${CPR.data}" varStatus="status">
					...
					<c:if test="${CP_SESSION.grade eq '본사' && CP_SESSION.role != '일반'}">
							<td class="btn-td">
								...
								<c:if test="${entry.status == '대기'}">
									<a class="btn green btn-sm vactSettle-pay-out">확정</a>
									<a class="btn red-haze btn-sm vactSettle-pay-hold">보류</a>
								</c:if>
								<c:if test="${entry.status == '보류'}">
									<a class="btn default btn-sm vactSettle-pay-cancel">대기</a>
								</c:if>
								<c:if test="${entry.status == '확정' && entry.payStatus == '대기'}">
									<a class="btn blue-soft btn-sm vactSettle-pay-cancel">확정 취소</a>
								</c:if>
							</td>
					</c:if>
					...
					<c:if test="${CP_SESSION.grade eq '본사' && CP_SESSION.role != '일반'}">
						<td><a class="btn green btn-sm vactSettle-detail" data-grade="stlAgencyId">엑셀 다운로드</a></td>
					</c:if>
