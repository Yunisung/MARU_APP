	...
	<div class="actions">
		...
		<div class="btn-group">
			...
			<ul class="dropdown-menu pull-right">
				<li><a href="javascript:;" class="vactSettle-pay-out-check is-mcht">
						<i class="fa fa-check-square-o"></i> 선택항목 확정 처리
					</a>
				</li>
				<li><a href="javascript:;" class="vactSettle-pay-complete is-mcht">
		...
		<div class="btn-group">
			...
			<ul class="dropdown-menu pull-right">
				<li><a href="javascript:;" class="vactSettle-pay-out-make is-mcht" data-bank="020">
						<i class="fa fa-check-square-o"></i> 우리은행
					</a>
				</li>
				<li><a href="javascript:;" class="vactSettle-pay-out-make is-mcht" data-bank="004">
						<i class="fa fa-check-square-o"></i> 국민은행
					</a>
				</li>
				<li><a href="javascript:;" class="vactSettle-pay-out-make is-mcht" data-bank="081">
			...
			<tbody id="list">
				...
				<c:forEach var="entry" items="${CPR.data}" varStatus="status">
					<tr data-stlId="${entry.stlId}"<c:if test="${status.index ne 0 && entry.stlDay ne CPR.data[status.index-1].stlDay}">
						...
						<td class="text-right"><fmt:formatNumber type="number" value="${entry.stlAmount}" pattern="#,##0" /></td>
					<c:choose>
					...
					<c:if test="${(CP_SESSION.grade eq '본사') && CP_SESSION.role != '일반'}">
						<td class="btn-td">
								...
								<c:if test="${entry.status == '대기'}">
									<a class="btn green btn-sm vactSettle-pay-out is-mcht">확정</a>
									<a class="btn red-haze btn-sm vactSettle-pay-hold is-mcht">보류</a>
								</c:if>
								<c:if test="${entry.status == '보류'}">
									<a class="btn default btn-sm vactSettle-pay-cancel is-mcht">대기</a>
								</c:if>
								<c:if test="${entry.status == '확정' && entry.payStatus == '대기'}">
									<a class="btn blue-soft btn-sm vactSettle-pay-cancel is-mcht">확정 취소</a>
								</c:if>
							</td>
					</c:if>
					<c:choose>
						...
						<c:if test="${(CP_SESSION.grade eq '본사') && CP_SESSION.role != '일반'}">
						<td><a class="btn green btn-sm vactSettle-detail" data-grade="stlId">엑셀 다운로드</a></td>
					...
					<tr>
						...
						<td class="text-right"><fmt:formatNumber type="number" value="${CPR.sum.stlAmount}" pattern="#,##0" /></td>
