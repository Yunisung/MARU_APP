								...
								<div class="page-content-inner" id="search-container">
									...
									<div class="portlet light portlet-form">
										<div class="portlet-body form light">
											<form class="form-horizontal" role="form" data-form="true" id="searchForm" name="searchForm" action="/loanSettle/status/list" method="post">
												<input type="hidden" data-reg="false" name="reason" value="대출현황조회">
												<c:if test="${CP_SESSION.grade == '본사'}">
													<input type="hidden" data-reg="false" name="thead" value="loanType:구분,loanId:대출ID,mchtId:가맹점ID,name:가맹점명,trxDay:일자,amount:대출금액,payAmt:납입금액,delayAmt:연체금액,balance:잔액,payCk:납입회차,delayCk:연체회차,loanStlId:대출정산번호">
												</c:if>
												<c:if test="${CP_SESSION.grade == '대행사' || CP_SESSION.grade == '에이전시' || CP_SESSION.grade == '지사'}">
													<input type="hidden" data-reg="false" name="thead" value="loanType:구분,mchtId:가맹점ID,name:가맹점명,trxDay:일자,amount:대출금액,payAmt:납입금액,delayAmt:연체금액,balance:잔액,payCk:납입회차,delayCk:연체회차">
												</c:if>
												<c:if test="${CP_SESSION.grade == '가맹점'}">
													<input type="hidden" data-reg="false" name="thead" value="loanType:구분,trxDay:일자,amount:대출금액,payAmt:납입금액,delayAmt:연체금액,balance:잔액,payCk:납입회차,delayCk:연체회차">
												</c:if>
												<div class="form-body">
													<div class="row">
														...
														<div class="form-group pg-form-group">
															<label class="control-label col-lg-4">일자</label>
															<div class=" col-lg-8">
														...
														<div class="form-group pg-form-group">
															...
															<select class="selectpicker col-lg-8" name="loanType" data-oper="eq">
																<option value="">-- 전체 -- </option>
																<option value="대출실행">대출실행</option>
