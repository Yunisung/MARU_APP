...
<div class="portlet-title">
	<div class="caption font-red-sunglo">
		...
		<span class="caption-helper amount_sum" style="color:#00a2ff;font-weight:600;">
		 		<span class="caption-detail"> 대출금액: <fmt:formatNumber type="number" value="${SUMMAP.amtSum}" pattern="#,##0" />원,</span>  
		 		<span class="caption-detail"> 상환금액: <fmt:formatNumber type="number" value="${SUMMAP.paySum}" pattern="#,##0" />원,</span>  
		 		<span class="caption-detail"> 잔액: <fmt:formatNumber type="number" value="${SUMMAP.balSum}" pattern="#,##0" />원</span>
		</span>
	</div>
	<div class="actions">
		...
		<table class="pg-table table table-bordered table-hover flip-content" id="sortTable">
			...
			<thead>
				<tr>
					...
					<th>영업자 이름</th>
					<th>정산주기</th>
					<th style="min-width:100px;">대출일자</th>
					<th style="min-width:100px;">상환완료일</th>
					<th>대출금액</th>
					<th>총상환금액</th>
					<th>잔액</th>
					<th>약정상환금액</th>
					...
					<th>납입횟수</th>
					<th>연체횟수</th>
					<th style="min-width:140px;">등록일시</th>
				</tr>
			</thead>
			<tbody id="list">
				...
				<c:forEach var="entry" items="${CPR.data}" varStatus="status">
					<tr>
						...
						<td>${entry.ceoName}</td>
						<td>${entry.submitName}</td>
						<td>${entry.settleType}</td>
						<td class="date">${entry.loanDay}</td>
						<td class="date">${entry.lastPayDay}</td>
					...
					<c:set var="totPayAmt" value="${totPayAmt + entry.totPayAmt}"/>
					<c:set var="balance" value="${balance + entry.balance}"/>
					<c:set var="conAmt" value="${conAmt + entry.conAmt}"/>
					<c:set var="conCnt" value="${conCnt + entry.conCnt}"/>
					<c:set var="paySession" value="${paySession + entry.paySession}"/>
					<c:set var="payCnt" value="${payCnt + entry.payCnt}"/>
					<c:set var="delayCnt" value="${delayCnt + entry.delayCnt}"/>
				...
				<tr  class="warning">
					...
					<td><fmt:formatNumber type="number" value="${totPayAmt}" pattern="#,##0" /></td>
					<td><fmt:formatNumber type="number" value="${balance}" pattern="#,##0" /></td>
					<td><fmt:formatNumber type="number" value="${conAmt}" pattern="#,##0" /></td>
					<td class="digits"><c:out value="${conCnt}"/></td>
					<td class="digits"><c:out value="${paySession}"/></td>
					<td class="digits"><c:out value="${payCnt}"/></td>
					<td class="digits"><c:out value="${delayCnt}"/></td>
					<td></td>
				</tr>
