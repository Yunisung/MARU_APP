	...
	<div class="actions">
		...
		<div class="btn-group">
			...
			<ul class="dropdown-menu pull-right">
				<li><a href="javascript:;" class="rent-settle-pay-out-check">
						<i class="fa fa-check-square-o"></i> 선택항목 확정 처리
					</a>
				</li>
				<li><a href="javascript:;" class="rent-settle-pay-complete">
					...
					<c:if test="${(CP_SESSION.grade eq '본사') && CP_SESSION.role != '일반'}">
						<td class="btn-td">
								...
								<c:if test="${entry.status == '대기'}">
									<a class="btn green btn-sm rent-settle-pay-out">확정</a>
									<a class="btn red-haze btn-sm rent-settle-pay-hold">보류</a>
								</c:if>
								<c:if test="${entry.status == '보류'}">
									<a class="btn default btn-sm rent-settle-pay-cancel">대기</a>
								</c:if>
								<c:if test="${entry.status == '확정' && entry.payStatus == '대기'}">
									<a class="btn blue-soft btn-sm rent-settle-pay-cancel">확정 취소</a>
								</c:if>
							</td>
					</c:if>
						...
						<td><a class="btn green btn-sm rent-settle-detail" data-grade="stlDistId">엑셀 다운로드</a></td>
					</tr>
				</c:forEach>
					<tr>
						<td></td>
