...
<body>
			...
			<div class="tab-pane active" id="tab1">
				...
				<form class="form-horizontal" role="form" id="transForm" action="#">
					...
					<div class="form-group">
						...
						<div class="col-md-4">
							<input type="text" name="amount" class="form-control" id="amount" value="" data-oper="comma" maxlength="10" value="0"> </div>
					</div>
	...
	<script>
		...
		$('#transferButton').on(
				'click',
				function () {
					console.log('ssssss' + $("#amount").val() + ${SUMAMT} + $('#transferType option:selected').val())
					var flag = false;
					if($('#transferType option:selected').val() === '실시간'){
						if($("#amount").val() != ${SUMAMT}) {
						...
						if ($('#pubDay').val().length != 8) {
							bootbox.alert('이체예정일자를 입력하여 주시기 바랍니다. ');
						} else if ($('#pubTime').val().length != 6) {
							bootbox.alert('이체예정시간을 입력하여 주시기 바랍니다.');
						} else if ($("#amount").val() != ${SUMAMT}) {
							bootbox.alert('금액을 확인하여 주시기 바랍니다.');
						} else {
							flag = true;
						}
					...
					if(flag) {
						$.ajax({
							type: "post",
							url: "/rent/trx/transfer/${TRXIDLIST}",
