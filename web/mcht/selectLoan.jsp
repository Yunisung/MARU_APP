									...
									<div class="portlet light">
										<div class="portlet-title">
											<div class="caption">
												...
												<span class="caption-title"> 가맹점  선정산 업체 등록 </span>
											</div>
										</div>
										...
										<div class="portlet-body form">
											<form class="form-horizontal form-bordered" role="form" data-form="true" id="writeFrm" name="form" action="/mcht/" method="post">
												...
												<div class="form-body row">
													<!-- 업데이트 구문 -->
													<input type="hidden" name="mchtId" value="${mchtId}" data-key="true" />
													<div class="form-group col-sm-12 form-subtitle">
														<label><i class="fa fa-reorder"></i> 선정산 업체 선택</label>
													</div>
													<div class="form-group col-sm-12">
														<label class="control-label input-sm col-sm-4 req-label">선정산 업체</label>
														<select name="loanId" class="selectpicker col-sm-6">
															<option value="">선택</option>
															<c:forEach var="entry" items="${DATALIST}" varStatus="status">
																<option value="${entry.loanId }">${entry.name} </option>
															</c:forEach>
														</select>
													</div>
												</div>
		...
		form1.validate({
            ...
            submitHandler: function (form) {
				...
				bootbox.confirm("입력하신 정보로 ${DATAMAP.name} 가맹점정보를 수정하시겠습니까?", function(result) {
					if (result) {
						ajaxFormSubmit(form, '/mcht/view/${mchtId}/tab_basic'); //PAGE 이동		
					}
				});
			}
		});
