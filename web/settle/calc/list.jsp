			...
			<tbody id="list">
				...
				<c:forEach var="entry" items="${CPR.data}" varStatus="status">
					<tr>
						...
						<td>${entry.mchtId}</td>
						<td title="${entry.taxId}">${entry.name}</td>
						<td class="text-right"><fmt:formatNumber type="number" value="${entry.amount}" pattern="#,##0" /></td>
						<td class="text-right"><fmt:formatNumber type="number" value="${entry.stlCount}" pattern="#,##0" /></td>
						<td class="text-right"><fmt:formatNumber type="number" value="${entry.stlFee}" pattern="#,##0" /></td>
						<td class="text-right"><fmt:formatNumber type="number" value="${entry.stlFeeVat}" pattern="#,##0" /></td>
				...
				<c:if test="${CPR.result.code == 200}">
					<tr class="lightgray">
						<td>*</td>
						<td colspan="4">합계</td>
						<td class="text-right"><fmt:formatNumber type="number" value="${SUMMAP.amount}" pattern="#,##0" /></td>
						<td class="text-right"><fmt:formatNumber type="number" value="${SUMMAP.stlCount}" pattern="#,##0" /></td>
						<td class="text-right"><fmt:formatNumber type="number" value="${SUMMAP.stlFee}" pattern="#,##0" /></td>
						<td class="text-right"><fmt:formatNumber type="number" value="${SUMMAP.stlFeeVat}" pattern="#,##0" /></td>
						<td class="text-right"><fmt:formatNumber type="number" value="${SUMMAP.stlAmount}" pattern="#,##0" /></td>
					</tr>
				</c:if>
			</tbody>
		</table>
