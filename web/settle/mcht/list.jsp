	...
	<div class="actions">
		...
		<div class="btn-group">
			...
			<ul class="dropdown-menu pull-right">
				<li><a href="javascript:;" class="settle-mcht-payout" data-status="지급완료">
						<i class="fa fa-check-square-o"></i> 선택항목 지급 완료 처리
					</a>
				</li>
				<li><a href="javascript:;" class="settle-mcht-payout" data-status="지급보류">
						<i class="fa fa-check-square-o"></i> 선택항목 지급 보류 처리
					</a>
				</li>
				<li><a href="javascript:;" class="settle-mcht-payout" data-status="삭제">
						<i class="fa fa-check-square-o"></i> 선택항목 삭제
					</a>
				</li>
			</ul>
		</div>
		<div class="btn-group">
			...
			<ul class="dropdown-menu pull-right">
				<li><a href="javascript:;" class="settle-pay-out-make" data-bank="020" data-grade="가맹점">
						<i class="fa fa-check-square-o"></i> 우리은행
					</a>
				</li>
				<li><a href="javascript:;" class="settle-pay-out-make" data-bank="004" data-grade="가맹점">
						<i class="fa fa-check-square-o"></i> 국민은행
					</a>
				</li>
				<li><a href="javascript:;" class="settle-pay-out-make" data-bank="081" data-grade="가맹점">
...
<div class="portlet-body form light">
	<div class="table-scrollable">
		...
		<c:if test="${CP_SESSION.grade eq '본사' }">
					...
					<th rowspan="3">정산번호</th>
					<th rowspan="3">지급상태</th>
					<th rowspan="3">정산일</th>
					<th style="min-width: 120px;" rowspan="3">가맹점명</th>
...
<c:set var="var17" value="0"/>
				<c:forEach var="entry" items="${CPR.data}" varStatus="status">
					...
					<c:if test="${status.index ne 0 && entry.stlDay ne CPR.data[status.index-1].stlDay}">
						...
						<td class="text-right digits">${(entry.payFee + entry.payVat + entry.payInterFee + entry.payInterVat) + (entry.rfdFee + entry.rfdVat + entry.rfdInterFee + entry.rfdInterVat) + (entry.relsFee + entry.relsVat)}</td>
		...
		<c:if test="${CP_SESSION.grade eq '본사'}">
						...
						<c:if test="${entry.status eq '지급대기'}">	
							<td class="text-right digits">
							<c:if test="${CP_SESSION.grade eq '본사'}">
								<a data-toggle="tooltip" title="${entry.summary}" href="javascript:changeDeductAmt('${entry.stlId}', '${entry.deductAmt}', '${entry.status}')">
								<c:if test="${entry.deductAmt eq '0'}"><i class="fa fa-plus"></i></c:if>
								<c:if test="${entry.deductAmt ne 0}"><fmt:formatNumber type="number" value="${entry.deductAmt}" pattern="#,##0" /> *</c:if>
								</a>
							</c:if>
						...
						<c:if test="${entry.status ne '지급대기'}">
							<td class="text-right digits" data-toggle="tooltip" title="${entry.summary}">
								${entry.deductAmt}
							</td>
						</c:if>
						
						
						<td class="text-right digits"><%--${entry.stlAmount + (entry.relsAmt - entry.relsFee - entry.relsVat) + entry.deductAmt} --%>
						<c:choose>
						...
						<c:choose>
							<c:when test="${entry.settleSvc eq '충전정산'}">
								<td colspan="3">가맹점충전잔액지급</td>
							</c:when>
							<c:otherwise>
						...
						<c:set var="var8"  value="${var8 +  (entry.payFee + entry.payVat + entry.payInterFee + entry.payInterVat) + (entry.rfdFee + entry.rfdVat + entry.rfdInterFee + entry.rfdInterVat) + (entry.relsFee + entry.relsVat)}"/>
		...
		<c:if test="${CP_SESSION.grade eq '본사'}">
						...
						<%--<c:set var="var14" value="${var14 + entry.relsAmt - entry.relsFee - entry.relsVat}"/> --%>
						<c:set var="var17" value="${var17 + entry.loanDeductAmt}"/>
						<c:set var="var15" value="${var15 + entry.deductAmt}"/>
						<c:choose>
						...
						<%--<c:set var="var16" value="${var16 + entry.stlAmount + (entry.relsAmt - entry.relsFee - entry.relsVat) + entry.deductAmt}"/> --%>
					</tr>
				</c:forEach>
				<tr>
						<td class="ck-td" ></td>
		...
		<c:if test="${CP_SESSION.grade eq '본사'}">
						...
						<%--<td class="text-right digits"><c:out value="${var10}" /></td> --%> <%-- ${entry.diffVanAmt} --%>
						...
						<td class="text-right digits"><c:out value="${var17}"/></td> <%-- ${entry.loanDecuctAmt} --%>
