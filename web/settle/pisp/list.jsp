		...
		<table class="pg-table table table-bordered table-hover flip-content" id="sortTable">
			...
			<thead>
				<tr>
					<th rowspan="3">No</th>
					<th rowspan="3">정산월</th>
					<th rowspan="3">가맹점아이디</th>
					<th colspan="11">거래내역</th>
				</tr>
				<tr>
					<th colspan="4">입금거래</th>
					<th colspan="4">출금거래</th>
					<th colspan="3">인증내역</th>
				</tr>
				<tr>
					<th style='width:99px'>건수</th>
					<th>금액</th>
					<th>수수료</th> 
					<th>원가</th>
					<th>건수</th>
			...
			<c:forEach var="entry" items="${CPR.data}" varStatus="status">
					<tr>
						...
						<td>${entry.thisMonth}</td>
						<td>${entry.mchtId}</td>
						<td><fmt:formatNumber value="${entry.inCnt}" type="number"/></td>
						<td><fmt:formatNumber value="${entry.inAmt}" type="number"/></td>
						<td><fmt:formatNumber value="${entry.inFee}" type="number"/></td>
						<td><fmt:formatNumber value="${entry.inVanFee}" type="number"/></td>
						<td><fmt:formatNumber value="${entry.outCnt}" type="number"/></td>
						<td><fmt:formatNumber value="${entry.outAmt}" type="number"/></td>
						<td><fmt:formatNumber value="${entry.outFee}" type="number"/></td>
						<td><fmt:formatNumber value="${entry.outVanFee}" type="number"/></td>
						<td><fmt:formatNumber value="${entry.fcsCnt}" type="number"/></td>
						<td><fmt:formatNumber value="${entry.fcsFee}" type="number"/></td> 
						<td><fmt:formatNumber value="${entry.fcsVanFee}" type="number"/></td>
					</tr>
			</c:forEach>
			</tbody>
		</table>
