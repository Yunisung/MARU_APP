...
<body class="page-header-fixed page-sidebar-closed-hide-logo page-content-white">
                                ...
                                <div class="btn-group btn-theme-panel">
                                    ...
                                    <div class="dropdown-menu theme-panel pull-right dropdown-custom hold-on-click panel-warning">
                                        ...
                                        <div class="panel-body">
                                            <p>* 매출일자, 승인번호, 거래번호가 입력된 엑셀파일을 업로드</p>
                                            <p>* 업로드한 엑셀에 맞는 가맹점명, 사업자번호가 검색되어 조회</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        	...
                        	<form class="form-horizontal form-bordered" role="form" data-form="true" id="writeFrm" name="form" action="" method="post">	
	                            <div class="form-body row">
		                            <div class="form-group col-sm-12 form-subtitle">
		                                <label><i class="fa fa-reorder"></i>거래내역 가맹점 정보조회</label>
		                            </div>
		                            
	                                <div class="form-group col-sm-12">
	                                    <label class="control-label col-sm-2">거래내역파일</label>
	                                    <div class="col-sm-8">
	                                    	<div class="fileinput fileinput-new" data-provides="fileinput">
		                                        <div class="input-group">
		                                            ...
		                                            <span class="input-group-addon btn blue btn-file">
		                                                <span class="fileinput-new"> Excel 거래내역파일 업로드 </span>
		                                            ...
		                                            <div class="pull-right">
                                                        <button type="button" class="btn btn-sm green right" id="search_submit" onClick="importExcelCollectData();">
                                                            <i class="fa fa-check" aria-hidden="true"></i> 가맹점정보 조회
                                                        </button>
                                                    </div>
		                                        </div>
		                                    </div>
                        ...
                        <div class="page-content-inner" id="search-container">
	                        <div class="portlet light portlet-form">
	                            <div class="portlet-body form light">
	                            	<form class="form-horizontal" role="form" data-form="true" id="searchForm" name="searchForm" action="/trx/list" method="post">
	                            	</form>
	                            </div>
	                        </div>
    ...
    <script type="text/javascript">
    						...
    						$(html).find('tr').each(function (i , e) {
    							if(i == 0) {
    								...
    								$(e).find('td').each(function (x , y) {
    									if(x == 0) {
    										if(flag) {
    											reqRow.trxDay = $(y).html();
    										}
    									}else if(x == 1) {
    										if(flag) {
    											reqRow.authCd = $(y).html();
    										}
    									}else if(x == 2) {
    										if(flag) {
    											reqRow.vanTrxId = $(y).html();
    											endFlag = true;
    										}
    									}
    	...
    	function importExcelCollectData() {
    		if(flag) {
    			bootbox.confirm('거래내역파일의 가맹점을 검색하시겠습니까?', function(result) {
    				if (result) {
    					$.ajax({
    						url: '/trx/excel/search',
    						dataType : "text",
    			...
    			bootbox.alert("정상적인 거래내역파일 등록해 주세요.");
    		}
    	}
    </script>
    <!-- 모달 생성을 위한 베이스 -->
