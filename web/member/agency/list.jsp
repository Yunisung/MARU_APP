		...
		<table class="pg-table table table-striped table-hover flip-content" id="sortTable">
			...
			<thead>
				<tr>
					...
					<th data-sort="string">소속 대행사</th>
					<th data-sort="string">사업자(주민)번호</th>
					<th data-sort="string">대표이메일</th>
			...
			<tbody id="list">
				...
				<c:forEach var="entry" items="${CPR.data}" varStatus="status">
					<tr <c:if test="${entry.payStatus == '미등록' || entry.payStatus == '중지'}">style="color:red;"</c:if>>
						<td>${CPR.page.total-((CPR.page.current-1)*CPR.page.size)-status.count+1}</td>
						<td class="link" data-url="/member/agency/view/${entry.agencyId}">${entry.agencyId}</td>
						<td class="link" data-url="/member/agency/view/${entry.agencyId}">${entry.name}</td>
						<td>${entry.distName}</td>
						...
						<td>${entry.regId}</td>
						<td>${entry.regDate}
							<c:if test="${entry.payStatus == '미등록'}">
							<Br>지불및 정산정보 미등록
							</c:if>
							<c:if test="${entry.payStatus == '중지'}">
							<Br>지불 중지
							</c:if>
						</td>
					</tr>
