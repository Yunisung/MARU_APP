		...
		<table class="pg-table table table-striped table-hover flip-content" id="sortTable">
			...
			<thead>
				<tr>
					...
					<th data-sort="string">권한</th>
					<th data-sort="string">기타거래</th>
			...
			<tbody id="list">
				...
				<c:forEach var="entry" items="${CPR.data}" varStatus="status">
					<tr>
						...
						<c:if test="${entry.showOthTrns ne 'Y'}">
							<td>미사용</td>
						</c:if>
						<td>${entry.status}</td>
						<c:if test="${entry.loanSettleStatus eq 'Y'}">
							<td>사용</td>
						</c:if>
						<c:if test="${entry.loanSettleStatus ne 'Y'}">
							<td>미사용</td>
						</c:if>
						<td>${entry.phone}</td>
						<td>${entry.regId}</td>
