		...
		<table class="pg-table table table-striped table-hover flip-content" id="sortTable">
			...
			<thead>
				<tr>
					...
					<th data-sort="string">적용 가맹점 수수료율</th>
					<th data-sort="string">현재 가맹점 수수료율</th>
					<th data-sort="string">적용 대행사 수수료</th>
					<th data-sort="string">현재 대행사 수수료</th>
					<th data-sort="string">적용 대행사 수수료율</th>
					<th data-sort="string">현재 대행사 수수료율</th>
					<th data-sort="string">적용 에이전시 수수료</th>
			...
			<tbody id="list">
				...
				<c:forEach var="entry" items="${CPR.data}" varStatus="status">
					<tr>
						...
						<td><fmt:formatNumber value="${entry.fee}"/> 원</td>
						<td><fmt:formatNumber value="${entry.beforeFee}"/> 원</td>
						<td><fmt:formatNumber value="${entry.rate * 100}" pattern="0.000"/> %</td>
						<td><fmt:formatNumber value="${entry.beforeRate * 100}" pattern="0.000"/> %</td>
						<td><fmt:formatNumber value="${entry.distFee}"/> 원</td>
						<td><fmt:formatNumber value="${entry.beforeDistFee}"/> 원</td>
						<td><fmt:formatNumber value="${entry.distRate * 100}" pattern="0.000"/> %</td>
						<td><fmt:formatNumber value="${entry.beforeDistRate * 100}" pattern="0.000"/> %</td>
						<td><fmt:formatNumber value="${entry.agencyFee}"/> 원</td>
						...
						<td>${entry.regId}</td>
						<td>${entry.regDate}</td>
						<c:if test="${entry.status !='완료' }">
						<td class="link" data-url="/member/vactRate/modify/${entry.idx }">변경</td>
						</c:if>
						<c:if test="${entry.status =='완료' }">
						<td>적용완료</td>
						</c:if>
