								...
								<div class="page-content-inner" id="search-container">
									...
									<div class="portlet light portlet-form">
										<div class="portlet-body form light">
											<form class="form-horizontal" role="form" data-form="true" id="searchForm" name="searchForm" action="/sales/mcht/list" method="post">
												<input type="hidden" data-reg="false" name="reason" value="가맹점 매출현황">
												<c:choose>
													<c:when test="${CP_SESSION.grade eq '본사' }">
														<input type="hidden" data-reg="false" name="thead" value="capDay:매입일,mchtName:가맹점,payAmt:매입 금액,payCnt:매입 건수,rfdAmt:매입취소 금액,rfdCnt:매입취소 건수,totalAmt:합계금액,totalCnt:합계건수,stlAmt:정산금액,stlFee:가맹점수수료,benefit:수익">
													</c:when>
													<c:when test="${CP_SESSION.grade eq '대행사' }">
														<input type="hidden" data-reg="false" name="thead" value="capDay:매입일,mchtName:가맹점,payAmt:매입 금액,payCnt:매입 건수,rfdAmt:매입취소 금액,rfdCnt:매입취소 건수,totalAmt:합계금액,totalCnt:합계건수,stlAmt:정산금액,stlFee:가맹점수수료,stlDistFee:대행사수익">
													</c:when>
													<c:when test="${CP_SESSION.grade eq '에이전시' }">
														<input type="hidden" data-reg="false" name="thead" value="capDay:매입일,mchtName:가맹점,payAmt:매입 금액,payCnt:매입 건수,rfdAmt:매입취소 금액,rfdCnt:매입취소 건수,totalAmt:합계금액,totalCnt:합계건수,stlAmt:정산금액,stlFee:가맹점수수료,stlAgencyFee:에이전시수익">
													</c:when>
							                        <c:when test="${CP_SESSION.grade eq '지사' }">
														<input type="hidden" data-reg="false" name="thead" value="capDay:매입일,mchtName:가맹점,payAmt:매입 금액,payCnt:매입 건수,rfdAmt:매입취소 금액,rfdCnt:매입취소 건수,totalAmt:합계금액,totalCnt:합계건수,stlAmt:정산금액,stlFee:가맹점수수료,stlSalesFee:지사수익">
													</c:when>
													<c:otherwise></c:otherwise>
												</c:choose><div class="form-body">
													<div class="row">
														...
														<div class="form-group pg-form-group">
															...
															<div class=" col-lg-8">
																<div class="input-group input-group-sm input-daterange" data-date-format="yyyy-mm-dd">
																	<input type="text" class="form-control now-date" name="capDay" value="" data-oper="ge">
