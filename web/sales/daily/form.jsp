...
<body class="page-header-fixed page-sidebar-closed-hide-logo page-content-white">
								...
								<div class="page-content-inner" id="search-container">
									...
									<div class="portlet light portlet-form">
										<div class="portlet-body form light">
											<form class="form-horizontal" role="form" data-form="true" id="searchForm" name="searchForm" action="/sales/daily/list" method="post">
												<input type="hidden" data-reg="false" name="reason" value="기간별 매출현황">
												<input type="hidden" data-reg="false" name="thead" value="capDay:매입일,saleAmount:매입 금액,saleCount:매입 건수,saleStlAmount:매입 정산금액,saleStlFee:매입 수수료,saleBenefit:매입 수익,rfdAmount:매입취소 금액,rfdCount:매입취소 건수,rfdStlAmount:매입취소 정산금액,rfdStlFee:매입취소 수수료 ,rfdBenefit:매입취소 수익,rfdedAmount:정산취소 금액,rfdedCount:정산취소 건수,rfdedStlAmount:정산취소 정산금액,rfdedStlFee:정산취소 수수료,rfdedBenefit:정산취소 수익,benefit:수익">
												<div class="form-body">
													<div class="row">
														<c:import url="/common/selectGrade.jsp" />
														<input type="hidden" id="grade_search" name="parentId" data-oper="eq" value=""/>
														<div class="form-group pg-form-group">
															<label class="control-label col-lg-4">매출기간</label>
															<div class=" col-lg-8">
	...
	<script type="text/javascript">
		...
		var theadVal = 'capDay:매입일,saleAmount:매입 금액,saleCount:매입 건수,saleStlAmount:매입 정산금액,saleStlFee:매입 수수료,';
		<c:choose>
			...
			<c:when test="${CP_SESSION.grade eq '지사' }">
				theadVal+='saleStlSalesFee:매입 수익,'
			</c:when>
		...
		<c:choose>
			...
			<c:when test="${CP_SESSION.grade eq '지사' }">
				theadVal+='rfdStlSalesFee:매입취소 수익,'
			</c:when>
			<c:when test="${CP_SESSION.grade eq '선정산' }">
		...
		<c:choose>
			...
			<c:when test="${CP_SESSION.grade eq '지사' }">
				theadVal+='rfdedStlSalesFee:매입취소 수익,salesFee:수익'
			</c:when>
			<c:when test="${CP_SESSION.grade eq '선정산' }">
