...
<div class="portlet-body form light">
	<c:set var="SALES_GRADE" value="대행사"></c:set> <%-- ${CP_SESSION.grade} --%>
			...
			<tbody id="list">
				<c:if test="${CPR.result.code == 200}">
					<tr class="lightgray">
                        ...
                        <c:choose>
                            <c:when test="${CP_SESSION.grade eq '본사' }">
                                <td><fmt:formatNumber type="number" value="${SUMMAP.benefit}" pattern="#,##0" /></td>
                            </c:when>
                            <c:when test="${CP_SESSION.grade eq '대행사' }">
				...
				<c:forEach var="entry" items="${CPR.data}" varStatus="status">
					<tr>
						...
						<c:choose>
                            <c:when test="${CP_SESSION.grade eq '본사' }">
                                <td><fmt:formatNumber type="number" value="${entry.benefit}" pattern="#,##0" /></td>
                            </c:when>
                            <c:when test="${CP_SESSION.grade eq '대행사' }">
