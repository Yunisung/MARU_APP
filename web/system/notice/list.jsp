...
<div class="portlet-title">
	<div class="caption font-red-sunglo">
		...
		<span class="caption-subject bold uppercase"> 공지사항 </span>
	</div>
	<c:if test="${CP_SESSION.grade =='본사' && CP_SESSION.role != '일반'}">
	<div class="actions">
		<a class="btn btn-sm green" href="/system/notice/add" data-original-title="" title=""><i class="fa fa-pencil" aria-hidden="true"></i> 공지사항 등록 </a>
	</div>
	</c:if>
</div>
		...
		<table class="pg-table table table-striped table-hover flip-content" id="sortTable">
			...
			<tbody id="list">
				<c:if test="${CPR.result.code != 200}">
					<tr>
						<td colspan="4">${CPR.result.code}:&nbsp;${CPR.result.message}:&nbsp;${CPR.result.error}</td>
					</tr>
				</c:if>
				<c:forEach var="entry" items="${CPR.data}" varStatus="status">
					<tr>
						<td>${CPR.page.total-((CPR.page.current-1)*CPR.page.size)-status.count+1}</td>
						<td class="link" data-url="/system/notice/view/${entry.idx}">${entry.title}</td>
						<td>${entry.regId}</td>
						<td>${entry.regDate}</td>
					</tr>
				</c:forEach>
