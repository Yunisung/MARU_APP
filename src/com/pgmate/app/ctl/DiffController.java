...
import com.pgmate.app.dao.CollectDiffDAO;
import com.pgmate.app.dao.CollectDiffKcpDAO;
import com.pgmate.app.dao.CollectDiffMchtDAO;
import com.pgmate.app.dao.CollectGroupDAO;
import com.pgmate.app.dao.DepositDAO;
import com.pgmate.app.dao.DiffTempDAO;
...
public class DiffController {
	...
	private static Logger logger = LoggerFactory.getLogger( com.pgmate.app.ctl.DiffController.class );
	
	@RequestMapping(value = {"/diff/excel/form"})
    public ModelAndView excelForm(HttpServletRequest request) {
        ...
        return new ModelAndView("/diff/excel/form");
    }
	
	@RequestMapping(value = "/diff/make/list", method = RequestMethod.POST,produces=MediaType.APPLICATION_JSON_VALUE)
...
//		RecordSet rset = dao.search();
		...
		sb.append("select A.*, IF(B.amount IS NOT NULL , 'TRUE', 'FALSE') as isSaved, B.amount as savedAmount,  B.diffVanAmount as savedDiffVanAmount, B.collectAmount as savedCollectAmount, B.deductAmount as savedDeductAmount, B.summary ");
		sb.append("from (select stlDiffVanDay,van,vanId,mchtId,name,MAX(tmnId) as tmnId, distId,agencyId,salesId, ");
		sb.append("SUM(amount) as amount, ");
		sb.append("SUM(stlDiffAmt) as stlDiffAmt, ");
		sb.append("SUM(stlDiffVanAmt) as stlDiffVanAmt, ");
		sb.append("SUM(stlDiffVanAmt-stlDiffAmt) AS MARUAmt from VW_TRX_CAP ");
		sb.append("WHERE stlDiffVanDay = '"+collectDay+"' ");
		sb.append("AND stlDiffStatus = '입금대기' ");
		...
		sb.append("group by stlDiffVanDay, van, vanId, mchtId) A ");
		sb.append("LEFT JOIN VW_COLLECT_DIFF_DTL B ON A.vanId = B.vanId AND A.mchtId = B.mchtId AND A.stlDiffVanDay = B.collectDay ");
		sb.append("WHERE B.collectId = '"+collectId+"' OR B.collectId IS NULL order by A.name asc");
		DAO dao = new DAO();
		RecordSet rset = dao.query(sb.toString());
		dao.initRecord();
		return new CPRUtil(cpRequest).dataList(rset, dao).setView(request,"/diff/make/list","");
	}
	
	@RequestMapping(value = "/diff/detail/list", method = RequestMethod.POST,produces=MediaType.APPLICATION_JSON_VALUE)
	public ModelAndView detailList(HttpServletRequest request,@RequestBody CPRequest cpRequest) {
		String collectId = cpRequest.getKeyValue("collectId");

		DAO dao = new DAO();

		dao.setTable("VW_COLLECT_DIFF_DTL");
		dao.setColumns("*");
		dao.addWhere("collectId", collectId);
		dao.setOrderBy("name asc");
		
		RecordSet rset = dao.search();
		return new CPRUtil(cpRequest).dataList(rset, dao).setView(request,"/diff/detail/list","");
	}
	
	@RequestMapping(value = {"/diff/temp/form"})
    public ModelAndView tempForm(HttpServletRequest request) {
		request.setAttribute("VAN_OPTION", new VanDAO().vanList().getRows());
		request.setAttribute("VANID_OPTION", new VanDAO().getVanId("").getRows());
		
        return new ModelAndView("/diff/temp/form");
    }
    
	
	@RequestMapping(value = "/diff/temp/list", method = RequestMethod.POST,produces=MediaType.APPLICATION_JSON_VALUE)
	public ModelAndView tempList(HttpServletRequest request,@RequestBody CPRequest cpRequest) {
		DiffTempDAO dao = new DiffTempDAO();
		if(cpRequest.getKeyValue("stlDiffVanDay").indexOf("-") > -1) {
			cpRequest.replaceKeyValue("stlDiffVanDay", cpRequest.getKeyValue("stlDiffVanDay").replaceAll("-", ""));
		}
		
		RecordSet rset = dao.makeSearch(cpRequest.data);
		return new CPRUtil(cpRequest).dataList(rset, dao).setView(request,"/diff/temp/list","");
	}
    
    @RequestMapping(value = {"/diff/collect/form"})
    public ModelAndView collectForm(HttpServletRequest request) {
		request.setAttribute("VAN_OPTION", new VanDAO().vanList().getRows());
		request.setAttribute("VANID_OPTION", new VanDAO().getVanId("").getRows());
		
        return new ModelAndView("/diff/collect/form");
    }
    
	
	@RequestMapping(value = "/diff/collect/list", method = RequestMethod.POST,produces=MediaType.APPLICATION_JSON_VALUE)
	public ModelAndView collectList(HttpServletRequest request,@RequestBody CPRequest cpRequest) {
		CollectDiffDAO collectDAO = new CollectDiffDAO();
		if(cpRequest.getKeyValue("stlDiffVanDay").indexOf("-") > -1) {
		...
		return new CPRUtil(cpRequest).dataList(rset, collectDAO).setView(request,"/diff/collect/list","");
	}
	
	@RequestMapping(value = "/diff/collect/view/{collectId}", method = RequestMethod.GET)
    public ModelAndView errView(HttpServletRequest request, @PathVariable String collectId) {
		DAO dao = new DAO();
		dao.setTable("VW_COLLECT_DIFF");
		dao.setColumns("*");
		dao.addWhere("collectId", collectId);
		...
		if(colMap.getString("status").equals("생성")) {
			return new ModelAndView("/diff/make/form");
		} else {
			return new ModelAndView("/diff/detail/form");
		}
    }
	
	@RequestMapping(value = "/diff/group/find/{idx}/{category}", method = RequestMethod.GET)
	...
	@RequestMapping(value = "/diff/group/find", method = RequestMethod.POST,produces=MediaType.APPLICATION_JSON_VALUE)
	...
	@RequestMapping(value = "/diff/add", method = RequestMethod.POST,produces=MediaType.APPLICATION_JSON_VALUE)
	public @ResponseBody SharedMap<String, Object> add(HttpServletRequest request,@RequestBody List<SharedMap<String, String>> requestList) {
		...
		for(SharedMap<String, String> eachMap : requestList) {
			...
			StringBuilder sb = new StringBuilder();
			sb.append("SELECT SUM(stlDiffVanAmt) as stlDiffVanAmt FROM VW_TRX_CAP ");
			sb.append("WHERE stlDiffVanDay = '"+eachMap.getString("collectDay")+"' ");
			sb.append("AND stlDiffStatus = '입금대기' ");
			...
			long stlDiffVanAmt = cpDAO.query(sb.toString()).getRowFirst().getLong("stlDiffVanAmt");
			cpDAO.initRecord();
    		...
    		dao.setRecord("stlVanAmount", stlDiffVanAmt);
	 ...
	 * 
	 */	
	@RequestMapping(value = "/diff/decide/{collectId}", method = RequestMethod.POST,produces=MediaType.APPLICATION_JSON_VALUE)
...
//			dao.setDebug(true);
			dao.setTable("PG_COLLECT_DIFF_DTL");
			dao.setColumns("idx");
			dao.addWhere("collectId", eachMap.getString("collectId"));
    		...
    		dao.setRecord("diffAmount", eachMap.getString("stlDiffAmt"));
    		dao.setRecord("diffVanAmount", eachMap.getString("stlDiffVanAmt"));
    		dao.setRecord("diffSettleAmount", eachMap.getString("diffSettleAmt"));
    		dao.setRecord("collectAmount", eachMap.getString("collectAmount"));
    		...
    		if(alreadyRest.size() > 0) {
				...
				dao.initRecord();
				dao.setTable("PG_COLLECT_DIFF_IDX");
				dao.addWhere("collectId", collectId);
				dao.addWhere("vanId", eachMap.getString("vanId"));
				dao.addWhere("mchtId", eachMap.getString("mchtId"));
				if(!dao.delete()) {
				...
				/*
				dao.setTable("PG_COLLECT_DIFF_RISK_IDX");
				dao.setColumns("*");
				dao.addWhere("collectId", collectId);
				dao.addWhere("vanId", eachMap.getString("vanId"));
				dao.addWhere("mchtId", eachMap.getString("mchtId"));
		...
		dao.addWhere("stlDiffVanDay", collectMap.getString("collectDay"));
		dao.addWhere("vanId IN ("+vanId+")");
		dao.addWhere("mchtId IN ("+mchtId+")");
		dao.addWhere("stlDiffStatus", "입금대기", DAO.eq);
//		dao.addWhere("risk", "", DAO.eq);
		RecordSet rsets = dao.search();
		if(rsets.size() > 0) {
			insertCollectDiffIdx(rsets.getRows(), collectId);
		}
		dao.initRecord();
		...
		if(riskCount > 0){
			...
			RecordSet rset2 = dao.search();
			if(rset2.size() > 0) {
				insertCollectDiffRiskIdx(rset2.getRows(), collectId);
			}
			dao.initRecord();
		}*/
		...
		// PG_COLLECT_DIFF의 collectAmt 업데이트
		CPDAO cpDAO = new CPDAO();
		String query = "SELECT SUM(collectAmount) AS collectAmt FROM PG_COLLECT_DIFF_DTL WHERE collectId = '"+collectId+"'";
		long collectAmt = cpDAO.query(query).getRowFirst().getLong("collectAmt");
		cpDAO.initRecord();
		cpDAO.update("UPDATE PG_COLLECT_DIFF set collectAmt = "+collectAmt+" WHERE collectId = '"+collectId+"'");
		cpDAO.initRecord();
	...
	@RequestMapping(value = "/diff/finish/{collectId}", method = RequestMethod.GET)
	public @ResponseBody SharedMap<String, Object> finish(HttpServletRequest request, @PathVariable String collectId) {
		...
		dao.initRecord();
		if(dao.update("UPDATE PG_COLLECT_DIFF SET status = '확정', deductAmount = '"+sumMap.getString("deductAmount")+"', remainingAmount = '"+sumMap.getString("remainingAmount")+"' WHERE status = '생성' AND collectId ='" + collectId + "'")) {
			resultMap.put("result", "OK");
		} else {
			resultMap.put("result", "NOK");
			resultMap.put("msg", collectId + " 업데이트에 실패했습니다.");
		...
		if(new TrxDAO().updateCollectedDiffCapDtl(collectId) > 0) {
			resultMap.put("result", "OK");
		} else {
			resultMap.put("result", "NOK");
			resultMap.put("msg", collectId + " 업데이트에 실패했습니다.");
		...
		if(new TrxDAO().updateCollectedDiffCapDtl(collectId, true) < 1) {
	...
	@RequestMapping(value = "/diff/delete/array", method = RequestMethod.POST,produces=MediaType.APPLICATION_JSON_VALUE)
	public @ResponseBody SharedMap<String, Object> deleteCollectArray(HttpServletRequest request, @RequestBody SharedMap<String, Object> reuqestMap) {
		...
		CollectDiffDAO collectDiffDAO = new CollectDiffDAO();
		for(String collectId : collectIds) {
			RecordSet rset = collectDiffDAO.getById(collectId);
	...
	@RequestMapping(value = "/diff/delete/{collectId}", method = RequestMethod.GET)
	...
	/* 마이너스 차액정산 생성 */
	@RequestMapping(value = "/diff/minus/add", method = RequestMethod.POST,produces=MediaType.APPLICATION_JSON_VALUE)
	public @ResponseBody SharedMap<String, Object> minusAdd(HttpServletRequest request,@RequestBody SharedMap<String, String> requestMap) {
		...
		dao.setRecord("stlVanAmount", requestMap.getString("diffVanAmount"));
		dao.setRecord("collectAmount", requestMap.getString("diffVanAmount"));
		dao.setRecord("collectAmt", 0);
	...
	public int insertCollectDiffIdx(List<SharedMap<String, Object>> capList, String collectId) {
		int inserted = 0;
		logger.debug("insert PG_COLLECT_DIFF_IDX batch : {}", capList.size());
		String query = "INSERT INTO `PG_COLLECT_DIFF_IDX` (`collectId`, `capId`, `vanId`, `mchtId`) " + "VALUES (?,?,?,?);";
		...
		try {
			...
			logger.debug("insert batch PG_COLLECT_DIFF_IDX error : {}", CommonUtil.getExceptionMessage(e));
		} finally {
			db.close(pstmt);
			db.close(conn);
		}
	...
	public int insertCollectDiffRiskIdx(List<SharedMap<String, Object>> capList, String collectId) {
		int inserted = 0;
		logger.debug("insert PG_COLLECT_DIFF_RISK_IDX batch : {}", capList.size());
		String query = "INSERT INTO `PG_COLLECT_DIFF_RISK_IDX` (`collectId`, `capId`, `vanId`, `mchtId`) " + "VALUES (?,?,?,?);";
		...
		try {
			...
			logger.debug("insert batch PG_COLLECT_DIFF_RISK_IDX error : {}", CommonUtil.getExceptionMessage(e));
		} finally {
			db.close(pstmt);
			db.close(conn);
		}
	...
	@RequestMapping(value = "/diff/mcht/list", method = RequestMethod.POST,produces=MediaType.APPLICATION_JSON_VALUE)
	public ModelAndView mchtList(HttpServletRequest request,@RequestBody CPRequest cpRequest) {
		CollectDiffMchtDAO settleMchtDAO = new CollectDiffMchtDAO();		
		RecordSet rset = settleMchtDAO.list(cpRequest.data,cpRequest.page);
		return new CPRUtil(cpRequest).dataList(rset, settleMchtDAO).setView(request,"/diff/mcht/list","");
	}
	
	@RequestMapping(value = "/diff/kcp/list", method = RequestMethod.POST,produces=MediaType.APPLICATION_JSON_VALUE)
	public ModelAndView kcpList(HttpServletRequest request,@RequestBody CPRequest cpRequest) {
		CollectDiffKcpDAO settleKcpDAO = new CollectDiffKcpDAO();		
		RecordSet rset = settleKcpDAO.list(cpRequest.data,cpRequest.page);
		return new CPRUtil(cpRequest).dataList(rset, settleKcpDAO).setView(request,"/diff/kcp/list","");
	}
}
