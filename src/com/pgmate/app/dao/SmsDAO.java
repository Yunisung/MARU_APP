...
public class SmsDAO extends DAO {
    private static Logger logger = LoggerFactory.getLogger( com.pgmate.app.dao.SmsDAO.class );
    ...
    public String getMchtName(String paykey) {
        String query = "SELECT * FROM PG_MCHT WHERE mchtId IN (SELECT mchtId FROM PG_MCHT_TMN WHERE payKey='"+paykey+"')";
        RecordSet rset = query(query);
        if(rset.size() == 0 ) {
            return "";
        } else {
            rset.next();
            return rset.getString("nick");
        }
    }
